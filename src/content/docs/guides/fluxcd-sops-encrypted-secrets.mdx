...
2. **Navigate to the directory with .sops.yaml**
   ...
   ```cmd
   cd secrets\Peercluster
   ```

3. **Encrypt the secret in place**

   ```cmd
   sops --encrypt --in-place some-secret.yaml
   ```
...
2. **Set up Age key environment variable**

   ```powershell
   [System.Environment]::SetEnvironmentVariable("SOPS_AGE_KEY_FILE", "C:\Users\YourUser\.sops\age.agekey", "User")
   ```
